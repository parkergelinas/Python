...
def tutorial():
    ...
    B2 = ttk.Button(
        tut,
        text="How do I trade with this client",
        command=lambda: popupmsg("Not yet supported"),
    )
    B2.pack()
    B3 = ttk.Button(
        tut,
        text="Indicator Questions/help",
        command=lambda: popupmsg("Not yet supported"),
    )
    B3.pack()
    tut.mainloop()
...
def addMiddleIndicator(what):
    ...
    if dataPace == "tick":
        popupmsg(
            "Indicators in Tick Data not available, choose 1 minute tf if you want short term."
        )
...
def changeTimeFrame(tf):
    ...
    if tf == "7d" and resampleSize == "1Min":
        popupmsg(
            "Too Much Data Chose, choose a smaller time frame or higher OHLC interval."
        )
    else:
        dataPace = tf
        counter = 9000
            ...
            if dataPace == "tick":
                try:
                    if exchange == "BTC-e":
                        ...
                        a2 = plt.subplot2grid(
                            (6, 4), (5, 0), rowspan=1, colspan=4, sharex=a
                        )
                        ...
                        a.plot_date(
                            sellDates, sells["price"], darkColour, label="sells"
                        )
                    ...
                    if exchange == "Bitstamp":
                        ...
                        data["datestamp"] = np.array(data["date"].apply(int)).astype(
                            "datetime64[s]"
                        )
                        ...
                        a.plot_date(
                            dateStamps, data["price"], lightColour, label="buys"
                        )
                    ...
                    if exchange == "Bitfinex":
                        ...
                        dataLink = (
                            "https://api.bitfinex.com/v1/trades/btcusd?limit=2000"
                        )
                        data = urllib.request.urlopen(dataLink)
                        data = data.read().decode("utf-8")
                        data = json.loads(data)
                    ...
                    try:
                        if exchange == "Huobi":
                            if topIndicator != "none":
                                ...
                                a = plt.subplot2grid(
                                    (6, 4), (1, 0), rowspan=5, colspan=4
                                )
                                a0 = plt.subplot2grid(
                                    (6, 4), (0, 0), sharex=a, rowspan=1, colspan=4
                                )
                            else:
                                a = plt.subplot2grid(
                                    (6, 4), (0, 0), rowspan=6, colspan=4
                                )
                            ...
                            if topIndicator != "none" and bottomIndicator != "none":
                                # actual price chart.
                                a = plt.subplot2grid(
                                    (6, 4), (1, 0), rowspan=3, colspan=4
                                )
                                # volume!
                                a2 = plt.subplot2grid(
                                    (6, 4), (4, 0), sharex=a, rowspan=1, colspan=4
                                )
                                # top indicator
                                a0 = plt.subplot2grid(
                                    (6, 4), (0, 0), sharex=a, rowspan=1, colspan=4
                                ...
                                # bottom indicator
                                a3 = plt.subplot2grid(
                                    (6, 4), (5, 0), sharex=a, rowspan=1, colspan=4
                                )

                            elif topIndicator != "none":
                                a = plt.subplot2grid(
                                    (6, 4), (1, 0), rowspan=4, colspan=4
                                )
                                a2 = plt.subplot2grid(
                                    (6, 4), (5, 0), sharex=a, rowspan=1, colspan=4
                                )
...
class BitcoinTracker(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        exchangeChoice.add_command(
            label="Bitfinex", command=lambda: changeExchange("Bitfinex", "bitfinex")
        )
        exchangeChoice.add_command(
            label="Bitstamp", command=lambda: changeExchange("Bitstamp", "bitstamp")
        )
        exchangeChoice.add_command(
        ...
        OHLCI.add_command(
            label="1 Minute", command=lambda: changeSampleSize("1min", 0.0005)
        )
        OHLCI.add_command(
            label="5 Minute", command=lambda: changeSampleSize("5min", 0.003)
        )
        OHLCI.add_command(
            label="15 Minute", command=lambda: changeSampleSize("15min", 0.008)
        )
        OHLCI.add_command(
            label="30 Minute", command=lambda: changeSampleSize("30min", 0.016)
        )
        ...
        tradeButton.add_command(
            label="Manual Trading", command=lambda: popupmsg("This is not live yet")
        )
        tradeButton.add_command(
            label="Automated Trading", command=lambda: popupmsg("This is not live yet")
        )

        tradeButton.add_separator()
        tradeButton.add_command(
            label="Quick Buy", command=lambda: popupmsg("This is not live yet")
        )
        tradeButton.add_command(
            label="Quick Sell", command=lambda: popupmsg("This is not live yet")
        )
        tradeButton.add_separator()
        tradeButton.add_command(
            label="Set-up Quick Buy and Sell",
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = ttk.Label(
            self,
            text="""Bitcoin Price Tracker is not responsible for any loss that is caused because the use of our application.
                   We kindly ask you to use our service as an educational tool only.""",
            font="Verdana 20",
        )
        ...
        button1 = ttk.Button(
            self, text="Agree", command=lambda: controller.show_frame(Graph_Page)
        )
        button1.pack()
